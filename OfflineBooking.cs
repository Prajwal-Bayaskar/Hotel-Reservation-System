...
{
    public partial class OfflineBooking : BaseForm
    {
        ...
        private void btnCancelBooking_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show(" Do you want to Cancel the booking?", "Cancel", MessageBoxButtons.YesNo) == DialogResult.No)
                return;
            con.Open();
            SqlCommand cmd = new SqlCommand("sp_bookingCancel", con);
        ...
        {
            this.Close();
            OnlineBookings ob = new OnlineBookings();
            ob.Show();
        ...
        private void cmbCustId_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                if (txtCustId.Text != null)
                {
                    ...
                    // Open the connection
                    con.Open();
                    SqlCommand cmd = new SqlCommand("sp_LoadCustRoomNo", con);
                    cmd.CommandType = CommandType.StoredProcedure;
